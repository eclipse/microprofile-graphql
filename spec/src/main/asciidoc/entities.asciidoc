...
This indicated that superPowers can be null, but if it's not, it has to have elements in the list.
...
Again, adding this only on the setter will only mark the input as non null and on the getter the type.
