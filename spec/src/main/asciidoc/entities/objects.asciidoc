...
If the entity cannot be serialized, the implementation must return in an internal server error to the client.
...
At the moment the spec does not support interfaces on input. 
